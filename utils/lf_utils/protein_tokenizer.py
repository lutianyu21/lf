...
from utils.dist_utils import DiscreteTokenizer, StructurePredictionModel, REFERENCE_FRAME
class DistMatrixTokenizer(ProteinTokenizer):
    def __init__(
        self,
        tokenizer_ckpt_path: str | Path,
        ...
        reference_frame = (REFERENCE_FRAME / self.structure_config.data.std_data).clone()
    ...
    def encode(self, protein: OpenfoldProtein, **kwargs) -> torch.Tensor:    
        ...
        if not torch.any(valid_mask):
            ca_coords = REFERENCE_FRAME[1].unsqueeze(0).clone()
            frame_coords = REFERENCE_FRAME.unsqueeze(0).clone()
        else:
            ca_coords = protein.residue_atom37_coord[:, atom_order['CA'], :][valid_mask]
            frame_coords = protein.residue_atom37_coord[:, frame_indices, :][valid_mask]

        def compute_local_pair_features(frame_coords: torch.Tensor, ch: int) -> torch.Tensor:
            """Return 4-channel local pair features derived from backbone frames."""
            ...
            diag_indices = torch.arange(dist_map.size(0))
            ...
            return torch.cat([dist_map * 21, v_local * 2], dim=-1)
        
        distance_tensor = compute_local_pair_features(frame_coords, ch=4)
        seq_len = distance_tensor.size(0)
        padded_len = math.ceil(seq_len / patch_size) * patch_size
    ...
    def decode(self, token_ids: torch.Tensor, **kwargs) -> OpenfoldProtein:
        ...
        quantizer = self.model.quantizer
        codes = quantizer.indices_to_codes(token_ids) # type: ignore
        codes = codes.to(self.device, dtype=torch.float32)
    ...
    def vsz(self) -> int:
        ...
        if levels is None:
            raise AttributeError("Quantizer does not expose discretization levels.")
