    ...
    def fn(self, batch: List[OpenfoldProtein]):
        if self.processor is None:
            ...
            qwen2_tokenizer: Qwen2TokenizerFast = AutoTokenizer.from_pretrained(
                GlobalConstants.working_dir / 'utils/qwen_utils/checkpoints/qwen3/Qwen3-0.6B'
            )
...
@ray.remote
class RawFileWorker:
    def __init__(self, df: pd.DataFrame, bsz: int):
        self.bq = df
        self.bsz = bsz
    ...
    def fn(self, queue: Queue):
        ...
        for _, row in self.bq.iterrows():
            path = GlobalConstants.auto_pathing(row['unirefAccession'])
